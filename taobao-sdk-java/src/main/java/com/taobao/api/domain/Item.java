 ...
 *
 * @author auto create
 * @since 1.0, 2010-10-28 10:43:37.0
 */
@ApiClass("Item")
public class Item extends TaobaoObject {

	private static final long serialVersionUID = 3896788673436878661L;
...
fee_card(话费软件代充)
	...
	/** 
	 * Item的发布时间,暂时仅供taobao.item.add使用
	 **/
	@ApiField("created")
...
taobao.items.search和taobao.items.vip.search专用
	...
	/** 
	 * 消保类型，多个类型以,分割。可取以下值：
2：假一赔三；4：7天无理由退换货；8：闪电发货；16：数码与家电30天维修
taobao.items.search和taobao.items.vip.search专用
	 **/
...
video.url等形式就只会返回相应的字段
	...
	/** 
	 * 商品30天交易笔数，只有调用商品搜索:taobao.items.get和taobao.items.search的时候才能返回
	 **/
	@ApiField("volume")
	private Long volume;
